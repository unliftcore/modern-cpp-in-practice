...
Every response carries method, path, status code, and latency. That is not yet production-grade structured logging -- the fields are embedded in formatted text rather than emitted as machine-queryable key-value pairs -- but it demonstrates the right instinct: capture identity (route), outcome (status), and timing (latency) at a single cross-cutting point. Evolving this toward a structured JSON or key-value emitter is a straightforward next step.
