...
These assertions serve as living documentation: if someone changes `Task` in a way that breaks the JSON contract, the compiler rejects the build immediately rather than deferring the failure to a runtime test or, worse, to production.
