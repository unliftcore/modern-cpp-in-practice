...
The example project in `examples/web-api/` embodies this pattern concretely. Its `main.cpp` constructs every long-lived resource in a single scoped sequence -- repository, router, middleware pipeline, server -- and wires them together before the process begins accepting work:
