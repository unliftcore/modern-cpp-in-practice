...
如果把每个类型都当成”附带方法的可变状态”来处理，现代 C++ 代码就会比它本该有的更难写。有些对象是值——它们代表一段自包含的含义，通常可以整体复制或替换，也应该便于比较和测试。有些对象则携带身份——它们对应一个特定的会话、账户、worker 或连接，即使字段随时间变化，其连续性仍然重要。如果在一个含糊的类型里混用这两种角色，就会冒出一堆看似无关的 bug：错误的相等性判断、不稳定的缓存键、意外的别名、糟糕的并发行为，以及说不清”改完之后到底还是不是同一个东西”的 API。

本章要讨论的，是如何让这两种分类保持清晰，并通过不变量的强制来保证类型在压力下仍然可信。参数传递和所有权转移机制不在本章范围内，那些内容属于别处。这里的重点是建模：什么时候应该把类型设计成值，什么时候必须显式保留身份，以及不变量如何防止对象图退化为一堆松散的字段。
...
道理说出来都懂。但一旦团队没有明确决定某个类型属于哪一类，设计上的伤害就会悄然出现。
...
这里有一条比很多代码库认识到的更有力的默认原则：如果一个类型不需要跨越时间的连续性，就优先把它设计成值。

## 值类型能减少意外耦合

值语义的威力在于消除了隐式共享。调用方拿到的是自己的副本或 move 过来的实例，修改只影响自身，相等性通常可以按结构判定。写测试时可以直接构造小例子，而不必搭建对象图或 mock 基础设施。
...
更深层的意义在于，值天然易于组合：可以放进容器、跨线程传递、用于确定性测试，也能充当稳定的哈希或比较输入。携带身份的对象也能做到这些，但需要付出更多规则和更多谨慎。只在模型真正需要的时候，才值得去承担这份复杂性。

## 不变量是拥有类型的根本理由

一个允许无效状态组合的类型，说到底不过是披着 struct 外衣的 bug 载体。

所谓不变量，就是对象在任何可被外部观察到的时刻都应成立的条件。比如时间窗口要求 `start <= end`；金额要求携带货币并使用有界整数表示；批处理策略要求 `max_items > 0` 且 `flush_interval > 0ms`；连接状态对象则禁止”已认证但未连接”。

不变量的意义不在于让构造函数显得更精巧，而在于大幅缩减后续代码需要防御的无效状态空间。
...
错误传递的细节留到下一章再展开，但建模的要点此处已经足够清楚：`RetryPolicy` 不应该以荒谬的状态存在。一旦创建成功，使用它的代码就不必再去检查延迟是否倒置、尝试次数是否为零——除非这些本身就是领域想要表达的合法含义。
...
这样一来，每个接收 `RetryPolicy` 的函数都得自行检查荒谬值，要么就寄希望于上游某层已经做过检查。实际情况往往是有的调用方查了，有的没查，导致行为随调用路径不同而不一致。前面展示的工厂做法则从结构上杜绝了这类 bug：只要你手里有一个 `RetryPolicy`，它就一定是有效的。

本书配套的 web-api 示例项目中，领域模型采用了同样的模式。`Task::validate()` 是一个返回 `Result<Task>` 的静态工厂，在边界处拒绝空标题或超长标题：
...
有一种反复出现的问题：某个类型看上去像值——会被复制、会被比较——但实际上内部藏着带身份的可变状态。
...
这个对象没法当成值来用——复制一个互斥量和一个活跃的取消标志毫无意义。它也没法当成严格的实体模型——因为整个可变表示都是 public 的。这种类型会把含糊性传染给代码库的其余部分。
...
本书配套的 web-api 示例项目清晰地展示了这种分离。`Task` 是纯粹的值类型——可复制、可比较、可序列化；而 `TaskRepository` 则是携带身份的实体，拥有 `shared_mutex`、ID 生成器以及可变集合。值承载领域数据，实体管理生命周期和同步，两者互不越界。

## 相等性应当匹配含义

检验一个类型的语义角色是否清晰，最好的办法之一就是看它的相等性是否不言自明。
...
本书配套的 web-api 示例项目在这一点上处理得很直接。`Task` 声明了默认三路比较，相等性完全基于结构：
...
这件事的影响是实实在在的。相等性会波及缓存键、去重逻辑、diff 生成、测试断言和变更检测。语义模糊的类型会产生语义模糊的相等性，进而同时拖垮多个子系统。
...
有经验的程序员有时会排斥小型包装类型，觉得跟普通整数或字符串比起来像是多余的仪式。但在生产级 C++ 中，这些类型往往很快就能收回成本。

`AccountId`、`ShardId`、`TenantName`、`BytesPerSecond`、`Deadline` 这样的类型，能杜绝参数传反、让日志更清晰，也让无效组合更难写出来。更重要的是，不变量和转换逻辑可以集中在类型内部，而不是散落在解析、存储和格式化代码各处。

但也要警惕：包装类型只有在真正使含义更精确时才有价值。如果只是在 `std::string` 外面套了个壳，所有无效状态照样存在，也没增加任何语义操作，那就只是噪音。该问的是：这个类型是否在强制或传达系统真正关心的某种区分？

## 当值保持为值时，并发会更容易

很多并发问题的根子其实是建模问题。共享可变状态之所以难对付，很大程度上是因为程序在本可使用不可变值的地方，用了携带身份的对象。

在流水线中传递一份经过验证的快照，理解起来很容易；而在同一条流水线上共享一个带内部锁的可变配置服务对象，就难得多了。把面向值的请求描述符投入工作队列，也远比传入一个带隐藏别名和同步机制的活跃会话对象简单。

这并不是说每个并发系统都能完全消除实体，而是说值语义是减少需要共享和同步的状态量的最有效手段之一。一旦代码能够用快照发布或值的消息传递来取代就地修改，正确性和可审查性都会随之提升。
...
当一个类型能清楚地回答”我到底是什么”时，其余设计都会变得顺畅：所有权更明确，API 更精练，测试更简单，并发也不用再和隐藏别名纠缠。这就是语义清晰需要在本书这么早出现的原因。
