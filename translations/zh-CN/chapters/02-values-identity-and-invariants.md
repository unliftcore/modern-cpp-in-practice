...
本章要讨论的，是如何让这两种分类保持清晰，并通过不变量的强制来保证类型在压力下仍然可信。参数传递和所有权转移机制不在本章范围内，那些留给其他章节。这里的重点是建模：什么时候应该把类型设计成值，什么时候必须显式保留身份，以及不变量如何防止对象图退化为一堆松散的字段。
...
这里有一条默认原则，比很多代码库意识到的更有力：如果一个类型不需要跨越时间的连续性，就优先把它设计成值。
...
不止于此——值天然易于组合：可以放进容器、跨线程传递、用于确定性测试，也能充当稳定的哈希或比较输入。携带身份的对象也能做到这些，但需要付出更多规则和更多谨慎。只在模型真正需要的时候，才值得去承担这份复杂性。
...
有个反复出现的问题：某个类型看上去像值——会被复制、会被比较——但实际上内部藏着带身份的可变状态。
