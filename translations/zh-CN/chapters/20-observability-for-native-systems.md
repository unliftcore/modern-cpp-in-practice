...
第一，不要把日志当成指标类问题的唯一信息来源。如果需要了解重试率或队列深度，应该直接发指标，而不是逼运维人员从文本里重新拼凑。第二，不要仅仅因为某次事故需要就把高基数负载或敏感数据全部记下来，应该放在专门的采样或调试路径中按需开启。
...
每条响应都携带了 method、path、状态码和延迟。这还不是生产级的结构化日志——字段嵌在格式化文本里，而非以机器可查询的键值对输出——但它体现了正确的直觉：在一个横切点上捕获身份标识（路由）、结果（状态码）和时序（延迟）。将其演化为结构化 JSON 或键值发射器是顺理成章的下一步。
