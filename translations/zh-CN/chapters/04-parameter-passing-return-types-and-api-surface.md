...
在 C++ 中，函数签名承载的信息往往超出作者的本意。它不仅说明了被调用方是借用还是保留数据，通常还隐含着 null 是否有意义、能否修改、会不会发生拷贝、失败算不算正常控制流等语义。一旦签名传达了错误的语义，哪怕实现本身在局部是正确的，API 用起来仍然代价高昂、审查起来仍然困难重重。

本章聚焦的正是这层语义接口。我们的目标不是死记”X 一律按 Y 传递”之类的教条，而是学会选择恰当的参数和返回形式，在调用方需要做决策的边界上，如实传达所有权、生命周期、可变性、可空性与开销。

这一边界视角使本章区别于前后各章。第 1 章讨论了谁拥有资源，第 2 章讨论了模型中有哪些对象，第 3 章讨论了错误应当如何跨越边界。本章的问题更具体，也更实用：在这些设计决策已经确定的前提下，函数签名该怎么写，才能让调用契约一目了然？

## 签名是契约，不是类型检查仪式

很多糟糕的 C++ API，根源在于把签名仅仅看作”能通过编译的最小类型集合”。这种做法忽略了一个事实：参数和返回值的选择本身就是带有约束力的文档。
...
函数接受一个指向栈 buffer 的 `string_view`，解析出 method、path、headers 和 body，返回一个成员全部为 `std::string` 的 `Request`——完全拥有自己的数据。调用方的 buffer 在函数返回后即可立即复用或销毁。这种"借用输入、拥有输出"的契约，仅从签名就能看清。

差别不在于风格是否讲究，而在于调用方能否不翻看实现就理解契约。
...
最常见的误用是让借用参数泄漏到长期状态中。一个接受 `string_view` 却把它缓存到调用结束之后的函数，不是什么巧妙设计，而是在对契约撒谎。
...
由此可得一条简洁实用的代码审查准则：凡是参数类型标明了借用，实现中若要保留数据，就必须通过显式拷贝或转换为拥有类型来完成。

## 当被调用方反正需要自己的副本时，就按值传递

现代 C++ 中最实用的模式之一，就是在被调用方需要存储或拥有参数时，直接按值传递。对于那些被灌输了”拷贝能省则省”观念的开发者来说，这往往令人意外。
...
准则不是”昂贵类型一律按 const 引用传递”，而是”当所有权转移给被调用方本身就是契约的一部分，且额外的 move/copy 开销可以接受时，按值传递即可。”

### 错误的参数选择及其成本

参数传递选错了，后果未必立竿见影，但在热路径和大对象场景下会不断累积。
...
非 const 引用参数是一种语义很强的表达。它意味着调用方必须提供一个存活的对象，null 没有意义，被调用方可以就地修改这个对象。这有时恰好是正确的契约，但也常常被滥用。

只有在修改是操作的核心目的，且调用方应当将其视为此次调用的主要意图时，才适合使用非 const 引用。典型场景包括：原地排序一个 vector、填充调用方提供的输出 buffer、推进解析器状态对象等。
...
同样的原则也适用于返回类型。在普通的现代 C++ API 中，返回拥有所有权的裸指针几乎总是错误的信号。而返回一个观察者裸指针则可以接受——前提是”缺失”本身有意义，且对象的生命周期由其他机制管理。

## 返回有意义的值，而非存储的副产品

返回类型应当和参数一样严谨。核心问题是：调用方应该得到一个拥有所有权的值、一个借用访问，还是一个像 `expected` 或 `optional` 那样承载决策信息的包装类型？

对于很多 API 来说，即使涉及一次 move，返回拥有所有权的值仍然是最干净的设计。它让生命周期保持局部化，让组合更容易，也避免了调用方对内部存储的依赖。C++23 的 move 语义已经让值返回在大多数场景下足够廉价，清晰性带来的收益完全值得。

借用返回类型只在源对象的生命周期显而易见、足够稳定，而且确实属于契约一部分时才适合。返回指向内部存储的 `std::string_view`，前提是该存储的生存期明显长于 view，且调用方可以安全地依赖这一点。在较宽的接口边界上，这通常不是好的权衡——因为它把生命周期推理的负担导出了被调用方本可自行消化的范围。
...
这类 API 在很多代码库中依然大行其道，因为它看起来很灵活。
...
创建函数正是所有权不清晰代价最高的地方。返回 `T*` 的工厂让调用方不得不追问：谁负责 delete？通过 out 参数加 bool 返回值的工厂，往往隐藏了部分构造的规则。默认返回 `shared_ptr<T>` 的工厂，则可能在设计尚未证明需要共享所有权时就强行引入了共享。
...
关键不在于具体选了哪种词汇类型，而在于：创建边界恰恰是所有权必须表达得毫无歧义的地方。

## API 接口也是成本接口

签名的选择会以调用方切实感受到的方式影响开销。

`std::function` 参数即使回调只在同步场景中使用，也可能带来堆分配和类型擦除的开销。`std::span<const T>` 能避免强迫调用方转换为特定容器。按值接收 `std::string` 的 sink 构造函数可以让临时对象高效 move 进来。返回拥有所有权的 vector 虽然分配一次内存，却能消除长期的生命周期隐患。这些都是设计层面的权衡，不是微优化。

正确的做法是：把调用方需要知道的成本摆在明面上，避免调用方无从推断的隐性开销。好的签名不承诺零成本，而是让重要的成本不会令人意外。

这也解释了为什么过于宽泛的”便利”重载集合反而有害。当一个 API 同时接受指针、字符串、span、vector、view 等各种组合时，重载接口本身可能比原始问题还难以理解。应当优先保留少数几个语义清晰的形式。

## 验证与评审

函数签名是发现设计缺陷成本最低、时机最早的环节之一。
...
测试应当验证签名所蕴含的语义，而不仅仅是核心功能。要验证是追加还是覆盖行为；验证返回的 view 在文档承诺的生命周期内有效，过期后确实失效；验证失败后输出参数或状态保持在承诺的条件下。再清晰的签名，也需要测试作为证据来支撑。
...
优秀的 C++ API 不只是能通过编译，而是足够早、足够清楚地表达意图，让调用方第一次阅读签名就能正确使用。签名值得投入这么多心思，原因正在于此。
