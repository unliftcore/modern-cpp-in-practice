...
这一边界视角让本章有别于前后各章。第 1 章讨论了谁拥有资源，第 2 章讨论了模型中有哪些对象，第 3 章讨论了错误应当如何跨越边界。本章的问题更具体，也更实用：在这些设计决策已经确定的前提下，函数签名该怎么写，才能让调用契约一目了然？
...
借用返回类型只在源对象的生命周期显而易见、足够稳定，而且确实属于契约一部分时才适合。返回指向内部存储的 `std::string_view`，前提是该存储的生存期明显长于 view，且调用方可以安全地依赖这一点。在较宽的接口边界上，这通常不是好的权衡——因为它把生命周期推理的负担推给了调用方，而这本是被调用方可以自行消化的。
