...
本章不是框架教程。要回答的生产问题更为具体：如果你希望六个月后回头看，所有权、失败处理、并发和运维依然清晰可审，那么一个小型 C++23 服务该是什么样子？答案不是”把最新特性全用上”，而是选择一种服务形态，让生命周期一目了然、异步工作归属明确、资源限制显式可控，并且在高压下仍能快速定位问题。

本章的示例是一个配置驱动的小型服务：接收请求、校验请求、执行有界的后台工作、持久化状态，并对外暴露指标和健康信息。细节刻意选得很普通——大多数生产服务在概念上并无新意，之所以出问题，是因为基本的工程边界没有划清。

## 先定义所有权单元，再定义部署单元

小型服务最常见的第一个架构错误，是按端点、处理器或框架回调来组织代码，而非按它拥有的资源来组织。一个可部署的服务拥有一组固定的长生命周期对象：配置、监听器、执行器、连接池、存储适配器、遥测汇聚点，以及关闭协调机制。如果不把这些对象显式建模出来，代码就会慢慢滑向全局变量、共享单例、脱管的后台工作，以及”听天由命”的关闭流程。
