...
标准库真正重要的时刻，不是充当工具层，而是它开始改变 API 所能表达的含义。在生产级 C++ 中，这种转变集中体现在一小组词汇类型上。它们使借用关系显式化，将缺失与失败区分开来，表达封闭的备选集合，并避免所有权信息散落在每个函数签名中。
...
函数从借来的输入中提取字段值，move 进拥有型字符串，返回一个完全自给自足的 `Task`。调用方的 buffer 在函数返回后可以立即复用或销毁。这正是前文所述的模式：检查时借用，存储时拥有。
...
假设一个配置加载器有三种可能的结果：没找到覆盖项、解析出了有效覆盖项、或者拒绝了格式错误的输入。三者在语义上截然不同。把它们硬塞进 `optional<Config>` 就丢掉了”格式错误为何被拒绝”的信息。返回 `expected<optional<Config>, ConfigError>` 看起来稍显沉重，但它精确表述了契约：缺失是正常的，格式错误才是失败。

在服务边界上，同样的精确性也至关重要。如果内部客户端库返回 `variant<Response, RetryAfter, Redirect>`，调用方就可以对合法的协议结果进行模式匹配。而如果返回的是 `expected<Response, Error>`，那么重试和重定向就算属于正常控制流，也会被错误地归入异常路径。

示例项目在领域边界上采用了这一方式。在 `examples/web-api/src/modules/error.cppm` 中，一个项目级别名让这一模式在整个代码库中保持一致：
