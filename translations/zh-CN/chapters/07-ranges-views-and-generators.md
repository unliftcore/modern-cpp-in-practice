...
值得思考的问题不是 range pipeline 是否优雅，而是：惰性组合在什么场景下比普通循环更能揭示工作的结构？又在什么场景下反而会掩盖所有权、错误处理和性能开销？C++23 提供了强大的 range 机制和用于拉取式序列的 `std::generator`，但它们都不应成为一切迭代的默认写法。
...
这就是好的 range 代码——pipeline 本身就是业务逻辑。没有棘手的就地修改，源对象的生命周期不存在歧义，末尾有一个清晰的物化点。中间存储在概念上就不需要存在，省掉它既提升了清晰度，也降低了开销。

再拿它和另一类循环比较：那种循环要更新共享计数器、发送 metric、原地修改记录，还要有条件地重试下游写入。在那种场景下，pipeline 反而会把最关键的部分——执行顺序和副作用——藏起来。一旦计算本身是有状态且副作用密集的，range 语法并不能提升可读性。
...
这是在局部算法内善用 range 工具的好例子：反向迭代的意图通过 `rbegin`/`rend` 清晰表达，而非依赖索引运算，函数产出的也是拥有型结果。
